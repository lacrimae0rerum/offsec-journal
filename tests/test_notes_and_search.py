...
def test_append_creates_markdown_block(tmp_env):
    note = notes.append("person", "santi", "Nota nueva sobre AD", author="fer", tags=["ad", "mentor"])
    assert note["author"] == "fer"
    read_back = notes.read_all("person", "santi")
...
def test_append_never_overwrites(tmp_env):
    notes.append("person", "santi", "primera", author="fer")
    notes.append("person", "santi", "segunda", author="fer")
    notes.append("person", "santi", "tercera", author="fer")
    blocks = notes.read_all("person", "santi")
...
def test_append_rejects_empty(tmp_env):
    ...
    with pytest.raises(ValueError):
        notes.append("person", "santi", "   ", author="fer")


def test_fts_finds_appended_note_after_sync(tmp_env):
    notes.append("project", "PT-2026-018", "Frontera AWS lambda pendiente", author="fer", tags=["aws"])
    sync.sync()  # rebuild FTS index
    with db.transaction() as conn:
        rows = conn.execute(
            "SELECT entity_id, body FROM notes_fts WHERE notes_fts MATCH ?",
            ('"lambda"',),
        ).fetchall()
...
def test_fts_tokenizer_handles_diacritics(tmp_env):
    ...
    with db.transaction() as conn:
        rows = conn.execute(
            "SELECT body FROM notes_fts WHERE notes_fts MATCH ?",
            ('"evolucion"',),
        ).fetchall()
    assert len(rows) >= 1
...
def test_queries_search_returns_grouped(tmp_env):
    ...
    with db.transaction() as conn:
        result = queries.search(conn, "hacking")
