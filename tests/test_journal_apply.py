...
def test_assign_apply_creates_row(tmp_env):
    ...
    journal.apply_entry(entry_id, "offsec", applied_by="test")
...
def test_reject_requires_reason(tmp_env):
    entry_id = _create_pending_assign("tbd_04", "PT-2026-018", 20)
    with pytest.raises(journal.JournalError):
        journal.reject_entry(entry_id, "offsec", "", applied_by="test")
    journal.reject_entry(entry_id, "offsec", "scope unclear", applied_by="test")
    with db.transaction() as conn:
        row = conn.execute(
...
def test_person_archive_is_soft_delete(tmp_env):
    entry = journal.create_entry("person_archive", {"id": "tbd_04", "archived": True}, "offsec", proposer="human")
