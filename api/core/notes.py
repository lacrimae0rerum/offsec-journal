"""Append-only markdown notes (decision #4), team-scoped.

Files live under notes/<team_slug>/persons/<id>.md, notes/<team_slug>/projects/<code>.md,
notes/<team_slug>/clients/<id>.md. Each note is a block:
...
def _target_path(entity_type: str, entity_id: str, team_slug: str) -> Path:
    sub = ENTITY_DIR.get(entity_type)
    if sub is None:
        raise ValueError(f"invalid entity_type '{entity_type}'")
    if not team_slug or "/" in team_slug or "\\" in team_slug or ".." in team_slug:
        raise ValueError(f"invalid team_slug: {team_slug!r}")
    # entity_id is attacker-controlled via request body — must not escape the
    # per-entity subdirectory. Reject path separators and traversal sequences.
    if not entity_id or "/" in entity_id or "\\" in entity_id or ".." in entity_id:
        raise ValueError(f"invalid entity_id: {entity_id!r}")
    base = settings.notes_dir / team_slug / sub
...
def append(
    ...
    path = _target_path(entity_type, entity_id, team_slug)
...
def read_all(entity_type: str, entity_id: str, team_slug: str) -> list[dict]:
